...
export default function MovieDetails({ setUser, user, setModal, modal }) {
    const [movie, setMovie] = useState(null)
    const [library, setLibrary] = useState("Add to Library")

    useEffect(() => {
        console.log(movie)
        const id = window.location.pathname.slice(window.location.pathname.indexOf('movie') + 6);


        axios.get(`https://api.themoviedb.org/3/movie/${id}?api_key=48586718f1619baec6911ced95941d83&language=en-US`)
            .then((r) => {
                if (user) {
                    axios.get('http://localhost:3000/api/prefrences', { headers: { userid: user._id } })
                        .then((response) => {
                            response.data.forEach((element, index) => {
                                if (element?.length) {
                                    element.forEach(e => {
                                        if (e._id == r.data.id && index === 0) {
                                            setLibrary("Planned")
                                        }
                                        else if (e._id == r.data.id && index === 1) {
                ...
                <div style={{ marginLeft: "50px" }}>
                    <div style={{ width: "50%", float: "left" }}>
                        ...
                        <div style={{
                            ...
                            <button style={{
                                ...
                                onClick={() => {
                                    ...
                                    axios
                                    axios.get(`https://youtube.googleapis.com/youtube/v3/search?key=${"AIzaSyCAvLgjLJQqWe-YXnon0FfujtmGIYzgMs8"}&maxResults=1&q=${movie.original_title}`)
                        ...
                        <div style={{
                            ...
                            <span style={{ marginLeft: "50px" }}>{movie && movie.release_date.slice(0, 4)}</span>
                        ...
                        <div style={{
                            ...
                            {movie && movie.genres.map((e, i) => {
                                if (i === 0) {
                                    return <span key={i}> {e.name + " "}&nbsp;{" |"}</span>
                                }
                                else if (i === movie.genres.length - 1) {
                        ...
                        <div style={{
                            ...
                            Languages
                            <div>
                                {movie && movie.spoken_languages.map((e, i) => {
                                    if (i === 0) {
                        ...
                        <div style={{
                            ...
                            Producers
                            <div>
                                {movie && movie.production_companies.map((e, i) => {
                                    if (i === 0) {
                    ...
                    <div>
                        <div style={{
                            ...
                            <h1 style={{ textAlign: "center" }}>{movie && movie.tagline}</h1>
                            <div style={{ width: "80%", marginLeft: "10%" }}>
                                {movie && movie.overview}
                            </div>
                            <div style={{ textAlign: "center", position: "relative", top: "100px", textDecoration: "underline" }}>
                                <a style={{ color: "white" }} href={movie && movie.homepage}>start watching</a>
                            </div>
                        </div>
                    </div>
                </div>
