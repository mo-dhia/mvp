...
export default function TrendingSeries({ tvS, trendingLeftArrowSeries, trendingRightArrowSeries }) {
  const [imgHover, setImgHover] = useState(null)
  const [leftHover, setLeftHover] = useState(null)
  const [rightHover, setRightHover] = useState(null)
  return (
    <div id="trendingtvS" style={{
      width: "90%", paddingBottom: "300px", position: "relative",
      display: "flex", alignItems: "center", left: "5%"
    }}>

      {tvS && tvS.counter - 8 !== 0 ?
        <div onClick={() => trendingLeftArrowSeries()} style={Object.assign({
      ...
      {tvS && tvS.data.map((e, i) => {
        var imgSrc = "https://image.tmdb.org/t/p/original" + e.backdrop_path
        return (
          <Link
            to={`/client/dist/index.html/serie/` + tvS.data[i].id}
            key={i}
      ...
      {tvS && tvS.counter < 21 ?
        < div onClick={() => trendingRightArrowSeries()} style={Object.assign({
          width: "100px", height: '300px',
          position: "absolute",
          color: "white",
          fontSize: "75px",
